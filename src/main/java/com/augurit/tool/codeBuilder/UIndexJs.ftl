...
return '
<a href="#" class="easyui-linkbutton ex-grid-operate" iconCls="icon-save" onclick="selectRow('+index+');editData()">修改</a>';
}
//${bean_desc}表格行删除操作
function format${vo_obj_name?uncap_first}Delete(val, row, index){
return '
<a href="#" class="easyui-linkbutton ex-grid-operate" iconCls="icon-remove" onclick="selectRow('+index+');deleteData()">删除</a>';
}
//表格选择行
